...
func generate_2d_points(radius: float, sample_region_shape, retries: int, start_pos := Vector2(INF, INF)) -> Array:
	_radius = radius
...
func _init_vars() -> void:
	...
	_cols = max(floor(int(_sample_region_rect.size.x / _cell_size)), 1)
	_rows = max(floor(int(_sample_region_rect.size.y / _cell_size)), 1)
